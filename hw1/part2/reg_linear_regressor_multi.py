...
class RegularizedLinearRegressor_Multi:
    ...
    def predict(self, X):
        ...
        ###########################################################################
        #y_pred = np.matmul(self.theta, np.transpose(X))
        y_ped = np.matmul(X, self.theta)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_pred
    ...
    def normal_equation(self,X,y,reg):
        ...
        Normal = np.matmul(X, np.linalg.pinv(np.matmul(np.transpose(X),X)))
        theta_n = np.matmul(y,Normal)
        ###########################################################################
        return theta_n
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        penalize = reg*np.dot(theta[1:], theta[1:]) 
        #diff = np.matmul(theta, np.transpose(X))-y
        diff = np.matmul(X, theta)-y
        J = (1/(2*num_examples))*(np.dot(diff,diff)+ penalize)              
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
    ...
    def grad_loss(self,*args):                                                                          
        ...
        #grad =np.dot(diff,X[:, -1])
        grad[1:] += reg*theta[1:]
        grad = (1/num_examples)*grad
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return grad
...
class LassoLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        #  2 lines of code expected                                               #
        ###########################################################################
        penalize = reg*np.linalg.norm(theta[1:], ord=1)
        diff = np.matmul(theta, np.transpose(X))-y
        J = (1/num_examples)*(0.5*np.dot(diff,diff)+ penalize)         
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
